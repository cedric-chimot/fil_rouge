...
if(isset($_POST['add_product'])){
    ...
    if($select_produits->rowCount() > 0) {
        ...
        $insert_produits= $conn->prepare("INSERT INTO `produits`(libelle, description, prix, image, categorie) VALUES(?,?,?,?,?)");
        $insert_produits->execute([$libelle, $description, $prix, $image, $categorie]);
...
if(isset($_GET['delete'])){
    ...
    $delete_cart = $conn->prepare("DELETE FROM `panier` WHERE idproduit = ?");
