...
if(isset($_POST['add_product'])){
    ...
    $ref_produit = intval($ref_produit);
    ...
    $select_produits = $conn->prepare("SELECT * FROM `produits` WHERE libelle = ?");
    ...
    if($select_produits->rowCount() > 0) {
        ...
        $insert_produits= $conn->prepare("INSERT INTO `produits`(ref_produit, libelle, description, prix, image, categorie) VALUES(?,?,?,?,?,?)");
        $insert_produits->execute([$ref_produit, $libelle, $description, $prix, $image, $categorie]);
...
if(isset($_GET['delete'])){
    ...
    $delete_product_image = $conn->prepare("SELECT * FROM `produits` WHERE id= ?");
    ...
    $delete_cart = $conn->prepare("DELETE FROM `panier` WHERE id = ?");
        ...
        <form action="" method="post" enctype="multipart/form-data">
            <div class="flex">
                <div class="inputbox">
                    ...
                    <input type="text" name="ref_produit" class="box">
                </div>
            ...
            ?>
                <!-- On les affiche ensuite dans le HTML -->
                        <div class="box">
                            ...
                            <h3>Référence produit</h3>
                            <div class="ref_produit"><?= $fetch_products['ref_produit']; ?></div>
                            <h3>Libellé</h3>
                            ...
                            <div class="flex-btn">
                                ...
                                <a href="produits.php?delete=<?= $fetch_products['id']; ?>" class="delete-btn"
